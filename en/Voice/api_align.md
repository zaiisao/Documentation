...
> [1] Agora Web SDK sets the channel profile by `ClientConfig` in `AgoraRTC.createClient` , see [ClientConfig](https://docs.agora.io/en/Voice/API%20Reference/web/interfaces/agorartc.clientconfig.html) for details.

> [2] Currently Agora Web SDK does not have a corresponding method to set the client role, but you can use the `Client.publish`, `Client.unpublish`, and `Client.subscribe` methods to switch roles between host and audience, see [Swtich the Client Role](../../en/Voice/role_web.md) for details.

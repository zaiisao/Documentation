...
| 功能 | 描述 | 
| ---------------- | ---------------- | 
| 实时音视频     | <% if (product == "1v1" || product == "1vn") { %>学生和教师可进行实时音视频互动。教师对学生讲课，学生就上课内容向老师提问，<br>形成良好的互动式课堂氛围。<% } if (product == "big") { %>教师对学生讲课，学生能实时接收老师的音频和视频。<br>教学过程中，学生可以举手请求发言，与老师进行互动。<br>所有学生都可以看到和听到互动学生和老师的画面及声音。<% } %>      | 
| 实时消息 | 学生和教师在课堂中发送实时文字消息进行互动。 |
| 白板 | 教师在白板上涂鸦、上传文件（PPT、Word 和 PDF）或播放视频，<br>有助于提炼教学重点，帮助学生理解或记忆。<% if (product == "1v1" || product == "1vn") { %>学生也可使用白板与老师进行实时互动。<% }  |
...
Agora 使用 Agora RTC SDK、Agora RTM SDK、Agora 云端录制服务、第三方白板 SDK 共同搭建一对一小班课场景。

![](https://web-cdn.agora.io/docs-files/1579589274133)

...
| 功能               | 描述                                                         |
| :----------------- | :----------------------------------------------------------- |
| 实时音视频         | 学生端能够发送和接收实时音视频数据。                         |
| 实时消息           | 学生端能够发送和接收实时消息。                               |
| 服务端推送媒体文件 | 服务端能够向频道内推送 MP4 文件，并支持播放队列，实现上课过程中视频片段的无缝切换。 |
| 声音降噪与增益     | 提供降噪工具，有效抑制背景噪声，并对声音进行增益。           |
| AI 补帧            | 提供 AI 补帧工具， 实现课程视频片段之间的无缝平滑切换。      |
| AI 口语测评        | 支持 AI 口语测评功能，对实时音视频流进行语音识别，提供多维度的语音评测结果，然后基于评测结果给予学生反馈。 |

### Demo 体验

Agora 为 AI 互动课堂提供如下平台的 Demo，扫描下方二维码即刻体验 AI 互动课堂。
...
1. 从 Agora SD-RTN™ 获取作答信息和口语评测结果，与题库比对判断对错。
2. 根据比对结果从视频库中获取对应的视频片段，通过 Agora Media Streaming Sever SDK 将视频片段推送给学生端。
...
声网通过智能调节码率和帧率、优化端到端全链路算法、运用抖动缓冲机制，确保 60% 丢包情况下，音视频通话流畅，80% 丢包情况下，音频通话流畅。
...
针对室内录播场景中常见的噪声、背景音等音质问题，声网声网通过自研的 AI 音频降噪算法，提供降噪工具，有效消除背景噪声，并对声音进行增益。

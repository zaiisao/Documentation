...
description: 
platform: Windows
updatedAt: Wed Sep 04 2019 10:41:46 GMT+0800 (CST)
---
# 媒体播放器组件
## 功能描述

当你使用 Agora Native SDK 实现视频通话时，如果进入频道后想将额外的一个视频也一同经由 SD-RTN 分发给远端用户观看，那么你需要使用媒体播放器组件播放这个额外的视频。你可以通过媒体播放器组件实时调节视频的播放情况。
...
- 你可以视频通话并将媒体播放器组件播放的视频也一同分发给远端用户。在这种场景下，你需要使用双进程。一个进程里采集并发送你通话时的视频，另一个进程里载入并发送播放器播放的视频。如果你只以一个进程加入频道，那么远端用户只能看到与这个进程对应的那个视频窗口。

...
description: 
platform: Unity
updatedAt: Fri Dec 13 2019 05:19:49 GMT+0800 (CST)
---
# 实现视频通话
本文介绍如何使用 Agora Unity SDK 快速实现视频通话，例如在游戏场景中实现自由发言。
...
- `channelKey`：传入能标识用户角色和权限的 Token。可设为如下一个值：
  ...
  - `NULL`
  - 临时 Token。在安全要求一般的测试场景下，可使用临时 Token。你可以在控制台里生成一个临时 Token（服务有效期为 24 小时），详见[获取临时 Token](https://docs.agora.io/cn/Agora Platform/token?platform=All Platforms#获取临时-token)。

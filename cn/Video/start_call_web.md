...
description: 
platform: Web
updatedAt: Tue Jul 21 2020 10:31:24 GMT+0800 (CST)
---
# 实现视频通话
...
1. 安装一款 Agora Web SDK 支持的浏览器，如下表所示：
    <div class="alert info">Agora 推荐使用最新版本的 Chrome 浏览器。</div>
...
![](https://web-cdn.agora.io/docs-files/1592907070303)
...
2. 在 `Client.init` 的 `onSuccess` 回调中调用 `Client.join` 加入频道。
   ...
   ```
	...
	- `token`: 该参数为可选。如果你的 Agora 项目开启了 App 证书，你需要在该参数中传入一个 Token，详见 [使用 Token](https://docs.agora.io/cn/Agora%20Platform/token?platform=All%20Platforms#使用-token)。
		...
		- 在生产环境，我们推荐你在自己的服务端生成 Token，详见 [生成 Token](../../cn/Video/token_server_cpp.md).

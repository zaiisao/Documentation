...
description: 
platform: macOS
updatedAt: Fri Sep 28 2018 20:13:18 GMT+0800 (CST)
---
# 实现视频通话
# 实现视频通话
...
1. 具体的开发环境要求、如何获取 App ID 以及 SDK 集成方法，详见 [设置开发环境](../../cn/Quickstart%20Guide/mac_video.md) 。
...
- 填入获取到的 [App ID](../../cn/Quickstart%20Guide/mac_video.md) 。只有 App ID 相同的应用程序才能进入同一个频道进行互通。
...
> - Agora 建议直接通过 `AgoraVideoProfile` 来定义输出视频的 Orientation 模式；`swapWidthAndHeight` 建议设为默认的 false。
...
现在你可以加入视频通话频道了。调用 `joinChannelByToken` 方法加入频道。同一个频道内的用户可以互相通话；多个用户加入同一个频道，可以群聊。
...
- [进阶：选择加密方案](../../cn/Quickstart%20Guide/encryption_mac_agora.md)
- [进阶：修改裸数据](../../cn/Quickstart%20Guide/rawdata_mac.md)

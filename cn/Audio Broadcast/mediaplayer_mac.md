...
description: 
platform: macOS
updatedAt: Wed Sep 04 2019 11:03:36 GMT+0800 (CST)
---
# 媒体播放器组件
## 功能描述
...
- 你可以以主播身份进入频道并将媒体播放器组件播放的视频也一同分发给远端用户。在这种场景下，你需要使用双进程。一个进程里采集并发送你作为主播的直播视频，另一个进程里采集并发送播放器播放的视频。如果你只以一个进程加入频道，那么远端用户只能看到与这个进程对应的一个视频窗口。
...
> - 图中仅展示最基本的接口，如果你有更复杂的需求，也可调用 Agora Native SDK 里更多的接口，比如[`enableDualStreamMode`](https://docs.agora.io/cn/Audio%20Broadcast/API%20Reference/oc/Classes/AgoraRtcEngineKit.html#//api/name/enableDualStreamMode:)。
